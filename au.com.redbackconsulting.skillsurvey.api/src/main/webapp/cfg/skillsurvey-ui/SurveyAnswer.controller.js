...
handleEdit : function (evt){
	...
	handleOverview:function(evt){
		var bus = sap.ui.getCore().getEventBus();
		bus.publish("nav","toOverview",{id : "SurveyAnswerOverview", tableid :"SurveyAnswer"	});
	},

	handleCreate:function(evt){
		var bus = sap.ui.getCore().getEventBus();
		bus.publish("nav","toCreate",{id : "SurveyAnswerCreate", tableid :"SurveyAnswer"	});
	},
	...
	sap.m.MessageBox.confirm(
				...
				function (oAction) {
					if (sap.m.MessageBox.Action.OK === oAction) {
						var bus = sap.ui.getCore().getEventBus();
						bus.publish("nav","toDelete",{id : "SurveyAnswer", tableid :"SurveyAnswer"});
					}
				}
		);
