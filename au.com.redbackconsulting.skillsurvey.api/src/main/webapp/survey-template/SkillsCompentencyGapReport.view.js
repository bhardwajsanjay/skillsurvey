sap.ui.jsview("survey-template.SkillsCompentencyGapReport", {

	/** Specifies the Controller belonging to this View. 
	* In the case that it is not implemented, or that "null" is returned, this View does not have a Controller.
	* @memberOf survey-template.SkillsCompentencyGapReport
	*/ 
	getControllerName : function() {
		return "survey-template.SkillsCompentencyGapReport";
	},
	...
	*/ 
	createContent : function(oController) {
		 ...
		 var oReportTable = new sap.ui.table.Table(
			...
			oReportTable.addColumn(needColumn);
			if(sap.ui.getCore().getModel("SkillsCompentencyGapReport").oData.length>0){
		...
		//	oReportTable.setGroupBy(needColumn);
			...
			oReportTable.setModel(sap.ui.getCore().getModel("SkillsCompentencyGapReport"));
		 ...
		 /////////////////////
			...
			oCell.addContent(new sap.ui.commons.Button({text:"Print",
				press:function(){
					...
					var skillsCompetencyGapReport = new SkillsCompetencyGap(sap.ui.getCore().getModel("ReportModel").oData);
					var 	width =document.documentElement.clientWidth-2;
					var reportPrint = window.open("", "MsgWindow", '"width='+width+',height=500, scrollbars=yes, resizable=no"');
					//var report = new Reports();
					reportPrint.document.write(skillsCompetencyGapReport.generateHTMLReport());
