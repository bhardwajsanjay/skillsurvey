sap.ui.jsview("survey-template.Supervisor", {

	/** Specifies the Controller belonging to this View. 
	* In the case that it is not implemented, or that "null" is returned, this View does not have a Controller.
	* @memberOf survey-template.Supervisor
	*/ 
	getControllerName : function() {
		return "survey-template.Supervisor";
	},
		...
		//	oQuestionTable.setGroupBy(uocName);
								...
								template : new sap.ui.commons.CheckBox(
										{
											...
											change : function(evt) {
												...
												var model = sap.ui.getCore().getModel("Supervisor").oData;
												...
												sap.ui.getCore().getModel("Supervisor").setData(model,false);
												...
												//	config.removeAndUpdateSecondaryQuestionByCouseId(this.getBindingContext().getObject().courseId, "Supervisor");
												//}
												//if(this.getBindingContext().getObject().isPrimary==2&&checkStatus==true ){
												//	config.getQuestionnaireByCouseId(this.getBindingContext().getObject().courseId, "Supervisor");
												//}
												
												
												oQuestionTable.setModel(sap.ui.getCore().getModel("Supervisor"));
								...
								template : new sap.ui.commons.CheckBox(
										{
											...
											change : function(evt) {
												...
												//	config.removeAndUpdateQuestionsByCouseId(this.getBindingContext().getObject().courseId, "Supervisor");
												//}
												//if(checkStatus==true&&this.getBindingContext().getObject().isPrimary==1){
												//	config.getSecondaryQuestionByCouseId(this.getBindingContext().getObject().courseId, "Supervisor");
												//}
												oQuestionTable.setModel(sap.ui.getCore().getModel("Supervisor"));
												oQuestionTable.bindRows("/");
											}
			...
			oCell.addContent(new sap.ui.commons.Button({text:'<Back',
				...
				press:function(){
					...
					if(!config.updateModelBeforeSaved("Supervisor")){
			...
			oCell.addContent(new sap.ui.commons.Button({
			  id:"button_mysurvey-Supervisor-subMenu",
				text:"Save",
				//enabled:config.getNeedSaveButtonStatus(),
				press:function(oEvent){
					//debugger;
					//config.updateModelBeforeSaved("Supervisor");
