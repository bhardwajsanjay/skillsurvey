...
//		 oQuestionTable.addColumn(courseColumn);
		...
		//	oQuestionTable.setGroupBy(uocName);
								...
								template : new sap.ui.commons.CheckBox(
										{
											...
											change : function(evt) {
												...
												if(checkStatus==true&&this.getBindingContext().getObject().isPrimary==1){
													config.removeAndUpdateSecondaryQuestionByCouseId(this.getBindingContext().getObject().courseId, "Desirable");
												}
												if(this.getBindingContext().getObject().isPrimary==2&&checkStatus==true ){
													config.getQuestionnaireByCouseId(this.getBindingContext().getObject().courseId, "Desirable");
												}
												
												
												oQuestionTable.setModel(sap.ui.getCore().getModel("Desirable"));
								...
								template : new sap.ui.commons.CheckBox(
										{
											...
											change : function(evt) {
												...
												if(checkStatus==true&&this.getBindingContext().getObject().isPrimary==2){
													config.removeAndUpdateQuestionsByCouseId(this.getBindingContext().getObject().courseId, "Desirable");
												}
												if(checkStatus==true&&this.getBindingContext().getObject().isPrimary==1){
													config.getSecondaryQuestionByCouseId(this.getBindingContext().getObject().courseId, "Desirable");
												}
												oQuestionTable.setModel(sap.ui.getCore().getModel("Desirable"));
												oQuestionTable.bindRows("/");
											}
			...
			oCell.addContent(new sap.ui.commons.Button({text:'<Back',
				...
				if(!config.updateModelBeforeSaved("Desirable")){
			...
			oCell.addContent(new sap.ui.commons.Button({
			  id:"button_mysurvey-Desirable-subMenu",
				text:"Save",
				//enabled:config.getNeedSaveButtonStatus(),
				press:function(oEvent){
					//debugger;
					//config.updateModelBeforeSaved("Desirable");
